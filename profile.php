    ...
    <head>
        <title><?php 
            ...
            while($row = $result->fetch_assoc()) {
                echo $row['username']."'s Profile - YT07";
            }
            $statement->close();
        ?></title>
        ...
        <div class="container-flex">
            <div class="col-2-3">
                <?php
                    ...
                    $result = $statement->get_result();
                    while($row = $result->fetch_assoc()) {
                        $finalstring = "<h3>Sobre ".$row['username']."</h3>
                        <img class=\"user-pic\" src=\"pfp/".getUserPic($row["id"])."\">
                        <div class=\"user-info\">
                            <div class=\"user-name\"><a href=\"profile.php?id=".$row["id"]."\">".$row["username"]."</a></div>
                            <div><span class=\"black\">".$row["subscribers"]."</span> suscriptores</div>";
                            if($_SESSION["subscribedto".$row["id"]] === false) {
                                $finalstring .= "<div><a href=\"subscribe.php?id=".$row["id"]."&u=0\">Subscribir</a></div>";
                            }
                            else{
                                $finalstring .= "<div><a href=\"subscribe.php?id=".$row["id"]."&u=1\">Desuscribir</a></div>";
                            }
                        $finalstring .= "</div>";
                    ...
                    while($row = $result->fetch_assoc()) {
                        ...
                        echo("Este utilizador no tiene videos.");
                    }
                    $statement->close();
                ?>
            </div>
